...
export function createSealife(island, shared, scene, camera, bommies = []) {
	...
	for (const sp of SPECIES) {
		...
		if (sp.reef) home.copy(overHead(bommies[Math.floor(r() * bommies.length)]));
		...
		for (const s of schools) {
			...
			if (s.timer <= 0) {
				...
				if (s.sp.reef) {
					...
					for (let j = 0; j < 6; j++) { const bm = bommies[Math.floor(Math.random() * bommies.length)], d = Math.hypot(bm.x - s.center.x, bm.z - s.center.z); if (d < bd && d > 2) { bd = d; best = bm; } }
					if (best) s.goal.copy(overHead(best));
				} else {
